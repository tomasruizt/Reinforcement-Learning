...
class SequentialGame:
    ...
    def __init__(self, agent: DiscreteAgent, environment: DiscreteEnvironment):
        """
        Setups a sequential Game for the input Agent in the input Environment.
        :param agent: The Agent that will take sequential decisions.
        :param environment: The Environment where the Agent will take decisions.
        """
        self._agent = agent
    ...
    def run_once(self) -> Iterable[DiscreteEpisode]:
        ...
        :return: The sequence of episodes that describe the sequential game just run.
        """
        episode_sequence = []
        state = self._environment.get_initial_state()
        while not self._environment.is_state_final(state):
    ...
    def _agent_makes_a_choice(self, from_state: DiscreteState) -> DiscreteEpisode:
        """
